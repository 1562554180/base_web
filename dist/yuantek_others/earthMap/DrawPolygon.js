...
function plotingStraightArrow(viewer, anchorpoints) {
  ...
  viewer.entities.add({
    name: 'StraightArrow',
    polygon: {
      hierarchy: bmgl.Cartesian3.fromDegreesArray(GeoPoints),
      material: bmgl.Color.RED.withAlpha(0.5),
      outline: true,
      outlineColor: bmgl.Color.GREEN,
      outlineWidth: 5,
    },
  });
}
...
function plotingRegularPolygon(viewer, anchorpoints) {
  ...
  viewer.entities.add({
    name: 'RegularPolygon',
    polygon: {
      hierarchy: bmgl.Cartesian3.fromDegreesArray(Geo_points),
      material: bmgl.Color.RED.withAlpha(0.5),
      height: 0,
      outline: true,
      outlineColor: bmgl.Color.GREEN,
...
function plotingRectangle(viewer, anchorpoints) {
  ...
  let GeoPoints = _createGeoPoints(viewer, window_points);
  console.log(bmgl.Cartesian3.fromDegreesArray(GeoPoints));
  viewer.entities.add({
    name: 'Rectangle',
    polygon: {
      hierarchy: bmgl.Cartesian3.fromDegreesArray(GeoPoints),
...
function _computeStraightArrowPoints(anchorpoints, ratio = 6) {
  ...
  if (anchorpoints[1].y > anchorpoints[0].y) {
    let arrowBased = {
      x: anchorpoints[0].x + (5 / 6) * Math.abs(anchorpoints[1].x - anchorpoints[0].x),
    ...
    let point4 = {
      x: arrowBased.x + (1 / 6) * Math.abs(anchorpoints[1].y - anchorpoints[0].y),
...
function _create_doublearrow(Anchorpoints) {
  ...
  let pointU_C = {
    x: ((pointU_1.x + pointU_2.x) * 5 + (pointU_3.x + pointU_4.x)) / 12,
    y: ((pointU_1.y + pointU_2.y) * 5 + (pointU_3.y + pointU_4.y)) / 12,
  };
  var pointC_l_out = calculateIntersectionFromTwoCorner(
    pointU_1,
    pointU_4,
    Math.PI / 8,
    Math.PI / 6
  ...
  var pointC_l_inner = calculateIntersectionFromTwoCorner(
    pointU_C,
    pointU_4,
    Math.PI / 8,
    Math.PI / 16
  ...
  var pointC_r_out = calculateIntersectionFromTwoCorner(
    pointU_2,
    pointU_3,
    Math.PI / 8,
    Math.PI / 6
  ...
  var pointC_r_inner = calculateIntersectionFromTwoCorner(
    pointU_C,
    pointU_3,
    Math.PI / 8,
    Math.PI / 16
  ...
  var pointC_l_inner_2 = {
    x: v_l_inner_1.x * d_l_a + pointU_4.x,
    y: v_l_inner_1.y * d_l_a + pointU_4.y,
  };

  var pointC_l_a_l = {
    x: pointC_l_out_2.x * 1.5 - pointC_l_inner_2.x * 0.5,
    y: pointC_l_out_2.y * 1.5 - pointC_l_inner_2.y * 0.5,
  };

  var pointC_l_a_r = {
    x: pointC_l_inner_2.x * 1.5 - pointC_l_out_2.x * 0.5,
    y: pointC_l_inner_2.y * 1.5 - pointC_l_out_2.y * 0.5,
  };
  ...
  var pointC_r_inner_2 = {
    x: v_r_inner_1.x * d_r_a + pointU_3.x,
    y: v_r_inner_1.y * d_r_a + pointU_3.y,
  };

  var pointC_r_a_r = {
    x: pointC_r_out_2.x * 1.5 - pointC_r_inner_2.x * 0.5,
    y: pointC_r_out_2.y * 1.5 - pointC_r_inner_2.y * 0.5,
  };

  var pointC_r_a_l = {
    x: pointC_r_inner_2.x * 1.5 - pointC_r_out_2.x * 0.5,
    y: pointC_r_inner_2.y * 1.5 - pointC_r_out_2.y * 0.5,
  };
  ...
  var v_U_4_3_l = {
    x: (v_U_4_3_.x * d_U_4_C) / (d_U_4_C + d_U_3_C),
    y: (v_U_4_3_.y * d_U_4_C) / (d_U_4_C + d_U_3_C),
  };
  var v_U_4_3_r = {
    x: (v_U_4_3_.x * d_U_3_C) / (d_U_4_C + d_U_3_C),
    y: (v_U_4_3_.y * d_U_3_C) / (d_U_4_C + d_U_3_C),
  };
  ...
  if (
    isPointInSegment(points_x, pointU_1, pointU_2) &&
    isPointInSegment(points_x, pointU_3, pointU_4)
  ) {
    let result = points_l;
    let result1 = [];
    let result2 = [];
...
function _computeBowPoints(anchor) {
  ...
  let o = { x: o_x, y: o_y };
  let r = Math.sqrt(
    (anchor[0].x - o_x) * (anchor[0].x - o_x) + (anchor[0].y - o_y) * (anchor[0].y - o_y)
  );
...
function _computeRegularPolygonPoints(anchor) {
  let r = Math.sqrt(
    (anchor[anchor.length - 1].x - anchor[0].x) * (anchor[anchor.length - 1].x - anchor[0].x) +
