(function(bmgl) {
  ...
  DrawBezierLine.prototype.initHandler = function() {
    ...
    handler.setInputAction(movement => {
      this.config.currentHash =
        Math.floor(movement.position.x) + '_' + Math.floor(movement.position.y);
    ...
    this.config.done.polyline.positions = new bmgl.CallbackProperty(() => {
      if (this.path.length > 1 && this.path[0].lng !== this.config.currentPosition.lng) {
        let p = _createGeoPoints(
          viewer,
          _createBezierPoints([...this.path, this.config.currentPosition])
        );
  ...
  DrawBezierLine.prototype.disable = function() {
    ...
    if (this.path.length >= 1) {
      ...
      draw.dom.dispatchEvent(
        new CustomEvent('bm_draw_end', {
          detail: {
            type: 'bezierLine',
  ...
  EditTailedAttackArrow.prototype.initHandler = function() {
    ...
    handler.setInputAction(e => {
      ...
      this.config.markers[this.config.index] &&
        (this.config.markers[this.config.index].position = this.config.markers[
          this.config.index
        ].position.getValue());
    ...
    this.config.done.polygon.hierarchy = new bmgl.CallbackProperty(() => {
      return new bmgl.PolygonHierarchy(
        xp.algorithm.tailedAttackArrow(this.path.map(v => [v.lng, v.lat])).polygonalPoint
      );
    }, false);
  ...
  EditPinchArrow.prototype.initHandler = function() {
    ...
    this.config.done.polygon.hierarchy = new bmgl.CallbackProperty(() => {
      return new bmgl.PolygonHierarchy(
        xp.algorithm.doubleArrow(this.path.slice(0, 4).map(v => [v.lng, v.lat])).polygonalPoint
      );
    }, false);
  ...
  Editpolyline.prototype.initHandler = function() {
    ...
    this.config.draft.polyline.positions = new bmgl.CallbackProperty(() => {
      return this.config.prevPosition && this.config.currentPosition
        ? [this.config.prevPosition.origin, this.config.currentPosition.origin]
        : bmgl.Cartesian3.fromDegreesArray([]);
    });
    handler.setInputAction(movement => {
      this.disable();
  ...
  EditStraightArrow.prototype.initHandler = function() {
    ...
    this.config.done.polygon.hierarchy = new bmgl.CallbackProperty(() => {
      return new bmgl.PolygonHierarchy(
        xp.algorithm.fineArrow(
          [this.path[0]['lng'], this.path[0]['lat']],
          [this.path[1]['lng'], this.path[1]['lat']]
        )
      );
    }, false);
  ...
  DrawRectangle.prototype.initHandler = function() {
    ...
    let callback = () => {
      if (this.config.prevPosition) {
        var p = [
            this.config.prevPosition.lng,
            this.config.prevPosition.lat,
            this.config.currentPosition.lng,
            this.config.currentPosition.lat,
          ],
          tmp;
  ...
  DrawPinchArrow.prototype.initHandler = function() {
    ...
    this.config.done.polygon.hierarchy = new bmgl.CallbackProperty(() => {
      if (this.path.length >= 2) {
        try {
          if (this.path.length === 2) {
            return this.path[1].lng !== this.config.currentPosition.lng
              ? new bmgl.PolygonHierarchy(
                  xp.algorithm.doubleArrow([
                    [this.path[0].lng, this.path[0].lat],
                    [this.path[1].lng, this.path[1].lat],
                    [this.config.currentPosition.lng, this.config.currentPosition.lat],
                  ]).polygonalPoint
                )
              : new bmgl.PolygonHierarchy([]);
            ...
            return this.config.draw
              ? new bmgl.PolygonHierarchy(
                  xp.algorithm.doubleArrow([
                    [this.path[0].lng, this.path[0].lat],
                    [this.path[1].lng, this.path[1].lat],
                    [this.path[2].lng, this.path[2].lat],
                    [this.config.currentPosition.lng, this.config.currentPosition.lat],
                  ]).polygonalPoint
                )
              : new bmgl.PolygonHierarchy(
  ...
  DrawDiagonalArrow.prototype.initHandler = function() {
    ...
    this.config.done.polygon.hierarchy = new bmgl.CallbackProperty(() => {
      if (this.path.length >= 1 && this.path[0].lng !== this.config.currentPosition.lng) {
        let p = _createGeoPoints(
          viewer,
          _computeDiagonalArrow([this.path[0], this.config.currentPosition])
        );
        return new bmgl.PolygonHierarchy(bmgl.Cartesian3.fromDegreesArray(p));
      } else {
        return new bmgl.PolygonHierarchy([]);
  ...
  DrawSwallowtailArrow.prototype.initHandler = function() {
    ...
    this.config.done.polygon.hierarchy = new bmgl.CallbackProperty(() => {
      if (this.path.length >= 1 && this.path[0].lng !== this.config.currentPosition.lng) {
        let p = _createGeoPoints(
          viewer,
          _computeSwallowtailArrow([this.path[0], this.config.currentPosition])
        );
        return new bmgl.PolygonHierarchy(bmgl.Cartesian3.fromDegreesArray(p));
      } else {
        return new bmgl.PolygonHierarchy([]);
  ...
  DrawMeasure.prototype.initHandler = function() {
    ...
    handler.setInputAction(movement => {
      ...
      if (area) {
        try {
          var r = bmgl.Rectangle.fromCartesianArray(
            this.config.done.polygon.hierarchy.getValue().positions
          );
          r = bmgl.Rectangle.center(r);
          this.labels.get(0).position = bmgl.Cartesian3.fromRadians(
            r.longitude,
            r.latitude,
            r.height
          );
    ...
    this.config.done.polygon.hierarchy = new bmgl.CallbackProperty(() => {
      if (this.path.length >= 2) {
        return new bmgl.PolygonHierarchy([
          ...this.path.map(v => v.origin),
          this.config.currentPosition.origin,
        ]);
      } else {
        return new bmgl.PolygonHierarchy([]);
      }
  ...
  DrawCircle.prototype.initHandler = function() {
    ...
    let callback = () => {
      return this.config.prevPosition
        ? getDistance(this.config.prevPosition, this.config.currentPosition)
        : 0;
    };

    this.config.done = viewer.entities.add({
      ellipse: Object.assign({}, { material: config.color }, config.ellipse, {
        semiMajorAxis: new bmgl.CallbackProperty(callback, false),
        semiMinorAxis: new bmgl.CallbackProperty(callback, false),
      }),
    });
  ...
  function getDistance(start, end) {
    return new bmgl.EllipsoidGeodesic(
      bmgl.Cartographic.fromDegrees(start.lng, start.lat),
      bmgl.Cartographic.fromDegrees(end.lng, end.lat)
    ).surfaceDistance;
  }
