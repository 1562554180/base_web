(function (bmgl) {
    ...
    function bmglHeatMap(viewer, data, config) {
        this.viewer = viewer;
    ...
    bmglHeatMap.prototype._parseData=function(data){
        ...
        base.canvas.height=Math.floor(base.canvas.width*(base.bounds.max.lat-base.bounds.min.lat)/(base.bounds.max.lng-base.bounds.min.lng));
        parseData.delta={
        ...
        data.map(v=>{
            ...
            parseData.data.push({x:base.canvas.delta+Math.floor(parseData.delta.x*(v.lng-base.bounds.min.lng)),y:Math.floor(base.canvas.height+base.canvas.delta-parseData.delta.y*(v.lat-base.bounds.min.lat)),value:value});
        });
    ...
    bmglHeatMap.prototype.fromLatLngToPoint=function(latLng) {
        let y=(1-Math.log(Math.tan(latLng.lat*Math.PI/180) + 1/Math.cos(latLng.lat*Math.PI/180))/Math.PI)/2 *256;
    ...
    bmglHeatMap.prototype.setData = function (data) {
        ...
        this.entity=this.viewer.entities.add({
            ...
            rectangle:{
                ...
                coordinates: bmgl.Rectangle.fromDegrees(
                    Math.max(base.bounds.min.lng-(base.bounds.max.lng-base.bounds.min.lng)*base.canvas.delta/base.canvas.width,-180),
                    Math.max(base.bounds.min.lat-(base.bounds.max.lat-base.bounds.min.lat)*base.canvas.delta/base.canvas.height,-90),
                    Math.min(base.bounds.max.lng+(base.bounds.max.lng-base.bounds.min.lng)*base.canvas.delta/base.canvas.width,180),
                    Math.min(base.bounds.max.lat+(base.bounds.max.lat-base.bounds.min.lat)*base.canvas.delta/base.canvas.height,90),
                ),
                outline:true,
                outlineWidth:10,
                outlineColor:bmgl.Color.YELLOW,
                materials:bmgl.Color.RED,
