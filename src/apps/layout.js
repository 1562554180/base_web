  ...
  */
  getRestartProgress = () => {
    ...
    rqHome.getRestartProgress({}, res => {
      ...
      this.setState({ progressWidth, stepItem, stepsCount: steps, otherUpdateVersion: restarts?.version })
      if (endTime && steps >= 22) {
  ...
  handleMenuClick = (item) => {
    ...
    if (menuKey.startsWith('download_') && menuKey.endsWith('_file')) {
      ...
      newWindow(`http://${window.location.host}/yuantek_assets/manual/usermanual.zip`, randomStr(20));
  ...
  joinClusterMasterFn = (values) => {
    const params =  {...values, rf_list: values.rf_list ? values.rf_list?.split(',').map(c => Number(c)) : []};
  ...
  renderNodeSet = () => {
    ...
    return (
      <Form labelCol={{span: 8}} wrapperCol={{span: 12}} onFinish={this.joinClusterMasterFn} style={{marginTop: 24}} initialValues={{slave_ip:config.isLocal ? config.localTestUrl:window.location.host}}>
        <Form.Item label="主节点IP" name="master_node_ip" rules={[{required: true}, {pattern: new RegExp(ipReg), message: '请输正确的IP格式'}]}>
          <Input />
        </Form.Item>
        <Form.Item label="当前节点IP" name="slave_ip" rules={[{required: true}, {pattern: new RegExp(ipReg), message: '请输正确的IP格式'}]}>
          <Input disabled />
        </Form.Item>
        <Form.Item label="RF" name="rf_list" rules={[{pattern: new RegExp(/^(\d+(,\d+)*)?$/), message: '必须以,隔开且不能用空格'}]}>
          <Input />
        </Form.Item>
        ...
        <Form.Item wrapperCol={{offset: 12}}>
          <Button loading={submitNodeSetLoading} type='primary' htmlType='submit' >确定</Button>
        </Form.Item>
      </Form>
    )
  }
  ...
  renderMatrixGain = () => {
    ...
    return <MatrixGainControl handleVisible={this.handleOpenMatrixGain} visible={matrixGainVisible} />
  }
    ...
    rqHome.handleGetWebConfig(params, (res) => {
      ...
      if (response?.code === 0) {
        ...
        const udata = data.map(i => {
          return { hidden: i.wk_type === 4 ? '1' : '0', config_name: i.config_name, vsat_id: i.vsat_id, wk_type: i.wk_type }
        }) || []
        this.props.dispatch({
          type: 'main/updateState',
          payload: { configAllNetList: udata, allNetList: data, allAdPort: response.all_ad_port_list || [] },
        })
      }
    })
  }
  ...
  setSystemSetting = (t) => {
    const mapping = { device_new_soft_restart: '软启动', device_new_restart: '重启', device_new_poweroff: '关机' };
    confirm({
      title: `是否${mapping[t]}？`,
      okText: '确定',
      cancelText: '取消',
  ...
  renderContent() {
    ...
    return (
      <Switch>
        {routers.map(this.authorizedRouteCreator)}
      </Switch>
    );
  }
  ...
  render () {
    const {
      ...
      hasLogined,
      clusterRow,
      selectedTheme, loginFlag, sendNetstationConfig,
    } = this.props;
    ...
    const netWorkCompositionProps = {
      settings,
      dispatch,
      handleModalVisible: (visible) => this.handleUpdateStateValue(visible, 'netWorkCompositionModalVisible'),
    }
