...
function getMemUsageClass(value) {
  ...
  if (percent >= 90) return 'value-error';
  if (percent >= 75) return 'value-warning';
  return 'value-normal';
}
...
export default function SystemResource({ data, cpuModules }) {
  ...
  const mergedComponents = React.useMemo(
    () => {
      ...
      const list = memComponents.map(mem => {
        const cpuMod = cpuModules?.find(mod => NAME_MAP[mod.title] === mem.name);
        return {
          name: mem.name,
      ...
      return list.sort((a, b) => COMPONENT_ORDER.indexOf(a.name) - COMPONENT_ORDER.indexOf(b.name));
    },
    [data, cpuModules]
  );
  ...
  return (
    <section className="module-section">
      <div className="module-header">系统资源</div>
          ...
          <div style={{ width: 'calc(50% - 6px)' }} className={styles['overview-block']}>
            <div className={styles['overview-header']}>
              ...
              <span className={styles['overview-stat']}>
                ...
                <span className={styles['overview-value']}>{cpuData.totalCores}</span>
              </span>
            </div>
            <div className={styles['overview-detail']}>
              <span className={styles['overview-label']}>总占用</span>
              <span
                className={`${styles['overview-value']} ${getCpuUsageClass(cpuData.systemUsage)}`}
              >
                {cpuData.systemUsage}%
              </span>
          ...
          <div style={{ width: 'calc(50% - 6px)' }} className={styles['overview-block']}>
            <div className={styles['overview-header']}>
              ...
              <span className={styles['overview-stat']}>
                ...
                <span className={styles['overview-value']}>{memTotalGB} GB</span>
              </span>
            </div>
            <div className={styles['overview-detail']}>
              <span className={styles['overview-label']}>占用</span>
              <span className={`${styles['overview-value']} ${getMemUsageClass(memSystemUsage)}`}>
                {memSystemUsage} GB
              </span>
              <span className="progress-bar">
                <span
              ...
              <div key={index} className={styles['resource-cell']}>
                ...
                <div className={styles['cell-metrics']}>
                  <div className={styles['cell-metric']}>
                    ...
                    <span
                      className={`${styles['metric-value']} ${getCpuUsageClass(comp.cpuUsage)}`}
                    >
                      {comp.cpuUsage}%
                    </span>
                  </div>
                  <div className={styles['cell-metric']}>
                    ...
                    <span className={styles['metric-value']}>{comp.memoryGB} GB</span>
                  </div>
                </div>
              </div>
            );
