...
export default function TopologyGV({
  ...
  function getSvgBoundsBox() {
    const container = topoRef.current;
    if (!container) return { width: 800, height: 600 };
    return { width: container.clientWidth, height: container.clientHeight };
  }

  function clampNodePos(id, x, y) {
    const isRf = id.startsWith('RF-');
    let w = 0,
      h = 0;
    if (isRf) {
      w = 14;
      ...
      const m = mods.find(n => n.id === id);
      if (m) {
        w = m.w;
        h = m.h;
      }
    }
    // Allow some overflow but keep node mostly visible
    const margin = 20;
    const svgBounds = getSvgBoundsBox();
    const maxX = (svgBounds.width - panX) / zoom + margin;
    const maxY = (svgBounds.height - panY) / zoom + margin;
    const minX = -panX / zoom - w - margin;
    const minY = -panY / zoom - h - margin;
    return {
  ...
  function loadLayout() {
    try {
      ...
      // Force mods/rfs useMemo to recalculate with restored offsets
      setDragTick(t => t + 1);
      return true;
    } catch {
      return false;
  ...
  function getSvgBounds(modsVal, rfsVal) {
    ...
    const allR = modsVal.map(n => n.x + n.w).concat(rfsVal.map(r => r.x + 14));
    const allY = modsVal.map(n => n.y).concat(rfsVal.map(r => r.y));
    const allB = modsVal.map(n => n.y + n.h).concat(rfsVal.map(r => r.y + 10));
  ...
  const dagreGraph = React.useMemo(
    () => {
      ...
      const nodeH = (ports, infoRows = 0) => Math.max(10, 4 + 2 + infoRows * 2 + ports * 2);
      ...
      chainsArr.forEach(c => {
        ...
        if (c.outType === 'AD') {
          ...
          const outId =
            typeof c.output === 'string' ? parseInt(c.output.split('-')[1]) : c.output?.id ?? 0;
      ...
      chainsArr.forEach(c => {
        ...
        if (c.converter != null && convertersArr.find(cv => cv.id === c.converter)) {
        ...
        if (outputId && !a[outputId]) {
          ...
          const card = isNaN(cardIdx) ? undefined : cardList.find(c => c.id === cardIdx);
      ...
      dvbCardsArr.forEach(card => {
        ...
        if (!a[outputId]) {
          const ports = Math.max(card.inputPorts || 1, dvbMaxPorts[card.id] || 1);
          a[outputId] = true;
          g.setNode(outputId, {
            width: nodeW(ports, 2, card.dna),
            height: nodeH(ports, 2),
    ...
    () => {
      ...
      chainsArr.forEach(c => {
        ...
        if (c.outType === 'AD') {
          ...
          const outId =
            typeof c.output === 'string' ? parseInt(c.output.split('-')[1]) : c.output?.id ?? 0;
          dvbMaxIn[outId] = Math.max(dvbMaxIn[outId] || 0, (c.inputPort ?? 0) + 1);
        }
      });
      ...
      g.nodes().forEach(v => {
        ...
        chainsArr.forEach(c => {
          if (!c.active) return;
          if (t === 'matrix' && String(num) === String(c.matrix?.deviceId ?? c.matrix?.id ?? '')) {
            const inIdx = c.matrixInPort ?? 0;
            const outIdx = c.matrixOutPort ?? 0;
            if (iP[inIdx]) iP[inIdx].on = true;
            if (oP[outIdx]) oP[outIdx].on = true;
          }
          if ((t === 'ad' || t === 'dvb') && getOutputId(c.output) === v && iP[c.inputPort ?? 0])
            iP[c.inputPort ?? 0].on = true;
          if (t === 'converter' && convertersArr.find(cv => cv.id === num) && iP[0])
            iP[0].on = true;
        });

        const prefix = { matrix: '矩阵', converter: '变频器', ad: 'AD', dvb: 'DVB' }[t];
        ...
        r.push({
          ...
          inPorts: iP,
          outPorts: oP,
          mx: t === 'matrix' ? matrixItemsArr.find(m => m.deviceId === matrixId) : null,
        });
      });
  ...
  const rfs = React.useMemo(
    ...
    [dagreGraph, rfPortsArr, dragTick]
  );
    ...
    () => {
      ...
      chainsArr.forEach(c => {
        ...
        if (t1) {
          const m1 = fm(t1);
          if (m1) {
            const pi = c.matrix ? c.matrixInPort ?? 0 : c.converter != null ? 0 : c.inputPort ?? 0;
        ...
        if (c.matrix && c.converter != null) {
          ...
          if (mxM && cvM) {
            ...
            const cpDist2 = Math.min(16, Math.max(8, (cvM.x - mxM.x - mxM.w) / 3));
            r.push({
              k: k++,
              d: `M ${mxM.x + mxM.w} ${oy} C ${mxM.x + mxM.w + cpDist2} ${oy} ${cvM.x -
                cpDist2} ${iy} ${cvM.x} ${iy}`,
            });
          }
        }
        ...
        if (outputId) {
          ...
          if (oM) {
            ...
            if (hp) {
              ...
              const cpDist3 = Math.min(16, Math.max(8, (oM.x - hp.x - hp.w) / 3));
              r.push({
                k: k++,
                d: `M ${hp.x + hp.w} ${oy2} C ${hp.x + hp.w + cpDist3} ${oy2} ${oM.x -
                  cpDist3} ${iy2} ${oM.x} ${iy2}`,
              });
            }
          }
        }
  ...
  React.useEffect(() => {
    ...
    hasRestoredLayoutRef.current = restored;
    // If no saved layout, auto-fit after a short delay
    if (!restored) {
  ...
  // After layout is restored and mods/rfs recalculated, center the view
  React.useEffect(
    () => {
      if (!hasRestoredLayoutRef.current) return;
      const timer = setTimeout(() => centerByCurrentZoom(mods, rfs, zoom), 50);
      return () => clearTimeout(timer);
      // eslint-disable-next-line react-hooks/exhaustive-deps
    },
    [dragTick]
  ...
  React.useEffect(
    () => {
      // Only reset layout if no saved layout was restored.
      // This prevents async data loading from wiping user-saved positions.
      if (hasRestoredLayoutRef.current) {
        return;
      }
      nodeOffsetsRef.current = {};
  ...
  function onMouseMove(e) {
    if (dragNode) {
      ...
      let rawX = sp.x - dragOffsetRef.current.x;
      let rawY = sp.y - dragOffsetRef.current.y;
      const clamped = clampNodePos(dragNode, rawX, rawY);

      if (isRf) {
        rfOffsetsRef.current = {
          ...rfOffsetsRef.current,
          [dragNode]: { x: clamped.x, y: clamped.y },
        };
      } else {
        nodeOffsetsRef.current = {
          ...nodeOffsetsRef.current,
      ...
      // Throttle React re-renders using rAF to avoid jitter
      if (!dragPendingRef.current) {
  ...
  function onMouseUp() {
    ...
    // Flush any pending drag update before clearing drag state
    if (dragPendingRef.current) {
      dragPendingRef.current = false;
      setDragTick(t => t + 1);
    }
  ...
  const cursorStyle = dragNode ? 'grabbing' : dragging ? 'grab' : 'default';
  ...
  return (
    ...
    >
                  ...
                  n.outPorts.length && (
                    <g>
                      {n.mx.internalConnections?.map((ic, ii) => (
                        <path
                          key={ii}
                          d={`M ${n.x} ${n.inPorts[ic.input]?.y} C ${n.x + n.w * 0.4} ${
                            n.inPorts[ic.input]?.y
                          } ${n.x + n.w * 0.6} ${n.outPorts[ic.output]?.y} ${n.x + n.w} ${
                            n.outPorts[ic.output]?.y
                          }`}
                          fill="none"
                          stroke="#f59e0b"
                          strokeWidth=".35"
            ...
            {rfs.map(rf => (
              ...
              >
                <rect
                  ...
                  height="10"
                  rx=".8"
                  fill={rf.online ? 'rgba(59,130,246,0.1)' : 'var(--bg-primary)'}
                  stroke={rf.online ? 'var(--device-rf)' : 'var(--border-color)'}
                  strokeWidth=".4"
                />
                <text
                  x={rf.x + 7}
                ...
                <text x={rf.x + 7} y={rf.y + 7} fill="#94a3b8" fontSize="2" textAnchor="middle">
                  {rf.lev}
                </text>
              </g>
            ))}
      ...
      >
        ...
        <button className={styles['dock-btn']} onClick={resetLayout} title="一键整理视图">
          <svg
            width="12"
            height="12"
            viewBox="0 0 16 16"
