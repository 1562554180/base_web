...
export default function TopologyGraph({ data, onViewModeChange }) {
  ...
  const signalChains = React.useMemo(() => {
    if (data?.chains && data.chains.length > 0) {
      return data.chains.map((c) => ({
        rf: c.rf,
        matrix: c.matrix
          ? matrixItems.find((m) => m.deviceId === c.matrix.id)
          : null,
    ...
    matrixItems.forEach((mx) => {
      mx.internalConnections?.forEach(({ input: inIdx, output: outIdx }) => {
        ...
        const inputPort = mx.outputPortMap
          ? mx.outputPortMap[outIdx] ?? outIdx
          : outIdx;
  ...
  const hasMatrix = React.useMemo(
    () => signalChains.some((c) => c.matrix !== null),
    [signalChains]
  );

  const hasConverter = React.useMemo(
    () => signalChains.some((c) => c.converter !== null),
    [signalChains]
  );
  ...
  const colSpanEmpty =
    5 + (hasMatrix ? 2 : 0) + (hasConverter ? 2 : 0);
      ...
      <div className={styles['module-content']}>
            ...
            >
              <thead>
                <tr>
                  ...
                  {hasMatrix && (
                    <>
                      <th className={styles['col-arrow']}></th>
              ...
              <tbody>
                {signalChains.map((chain, idx) => (
                  <tr
                    key={idx}
                    className={
                      !chain.active ? styles['chain-inactive'] : undefined
                    }
                  >
                    ...
                    <td className={styles['col-src']}>
                      {chain.rf !== null ? (
                        <span
                          className={`${styles['node-tag']} ${styles['rf-tag']}`}
                        >
                          <span>RF-{chain.rf}</span>
                    ...
                    {hasMatrix && (
                      <>
                        ...
                        <td className={styles['col-matrix']}>
                          {chain.matrix ? (
                            <span
                              className={`${styles['node-tag']} ${styles['matrix-tag']}`}
                            >
                              <span>{chain.matrix.id}</span>
                              <span className={styles['tag-detail']}>
                                IN{chain.matrixInPort}&#8594;OUT
                                {chain.matrixOutPort}
                              </span>
                            </span>
                          ) : (
                      ...
                      <>
                        ...
                        <td className={styles['col-conv']}>
                          {chain.converter !== null ? (
                            <span
                              className={`${styles['node-tag']} ${styles['conv-tag']}`}
                            >
                              <span>CV-{chain.converter}</span>
                              <span className={styles['tag-detail']}>
        ...
        {viewMode === 'table' && (
          <div className={styles['topology-legend']}>
            ...
            <span className={styles['legend-item']}>
              <span
                className={`${styles['status-dot']} ${styles['legend-dot']} ${styles['active']}`}
              ></span>{' '}
              已连接
            </span>
            <span className={styles['legend-item']}>
              <span
                className={`${styles['status-dot']} ${styles['legend-dot']} ${styles['empty']}`}
              ></span>{' '}
              未连接
            </span>
          </div>
