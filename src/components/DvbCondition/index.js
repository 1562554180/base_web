...
export default class DvbCondition extends Component {
  ...
  componentDidMount() {
    ...
    if (
      formConfig.some((item) => item.field_name === 'filter_gain_dB') &&
      !_.isEmpty(dvbguardFilterInfo) &&
      !_.isUndefined(dvbguardFilterInfo)
    ) {
      ...
      this.setState({
        formConfig: uform,
        filterMode: dvbguardFilterInfo?.filter_mode,
        filterGainDB: dvbguardFilterInfo?.filter_gain,
        gainDb: dvbguardFilterInfo?.filter_gain,
      });
    }
  }
  ...
  saveSetDvb = () => {
    ...
    const send = {
      ...
      filter_gain_dB: parseInt(this.remove(gainDb || ''), 10),
  ...
  render() {
    ...
    dvbguardFilterInfo.filter_mode = modeList.find(
      (item) => item.value === this.props.global.dvbguardFilterInfo?.filter_mode
    )?.label;
    return (
      <>
        <Card title="DVB板卡设置" style={{ marginBottom: '10px' }}>
          <Form>
            {formConfig?.map((item) => {
              return (
                <FromItemCreator
                  ...
                  item={{
                    ...item,
                    formItemLayout,
                    props: { onChange: (v) => this.updateState(v, item.field_name) },
                  }}
                />
              );
            })}
          ...
          <div style={{ marginTop: '10px', marginBottom: '15px', marginLeft: '20px' }}>
            <Button type="primary" onClick={this.saveSetDvb}>
              保存
            </Button>
          </div>
        </Card>
          ...
          extra={
            <div>
              <EditHeader
                ...
                updateHeaders={this.updateNowDvbFields}
                title={<Icon type="setting" title="DVB调理显示设置" />}
                modalTitle="DVB调理参数配置"
                type="now_dvb_use"
                style={{ display: this.props.isSuperAdministrator ? 'inline-block' : 'none' }}
              />
            </div>
          }
        >
