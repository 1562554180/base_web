...
const disabledSelectFields = ['名称', '节点显示属性', '节点分类', '关系分类', '纬度', '经度']
...
export default class EditTable extends PureComponent {
  constructor(props) {
    ...
    this.state = {
      ...
      searchValue: '',
      isUpdate: false,
      labelMapping: this.initlabelMapping(props.allNodeAttrs),
  ...
  initlabelMapping = (list) => {
    let newList = [];
    if (_.isArray(list)) {
      newList = list;
    } else if (!_.isEmpty(list)) {
      const { node_attr: nodeAttr, relation_attr: relationAttr } = list || {};
      newList = [...nodeAttr, ...relationAttr];
    }
  ...
  initColumns = (props) => {
    const oldColumns = props.columns || [
      ...
      {title: '中文名', width: 180, key: 'readable_name', dataIndex: 'readable_name'},
    ...
    const columns = [{title: '编号', dataIndex: '_id', key: '_id', width: 50, render: (t, r, i) => i + 1}];
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!_.isEqual(this.props.allNodeAttrs, nextProps.allNodeAttrs)) {
      this.setState({labelMapping: this.initlabelMapping(nextProps.allNodeAttrs)})
    }
  ...
  createPopvSelect = (item, r, i) => {
    const { allNodeAttrs = [] } = this.props;
    let newList = [];
    if (_.isArray(allNodeAttrs)) {
      newList = allNodeAttrs;
    } else if (!_.isEmpty(allNodeAttrs)) {
      const { node_attr: nodeAttr, relation_attr: relationAttr } = allNodeAttrs || {};
      newList = [...nodeAttr, ...relationAttr];
    }
    const options = [];
    for (const it of newList) {
      if (disabledSelectFields.includes(it.label)) continue;
      options.push(<Option key={it.label} type={it.field_type} value={it.label}>{it.label}</Option>)
    }
    return (
      <Select showSearch onChange={e => this.updateData(e, r, i, item.dataIndex)} style={{width: 150}}>
        {options}
      </Select>
    )
  }

  createPlusBtn = (item, r, i) => {
    return (
      <Popover trigger='click' content={this.createPopvSelect(item, r, i)} title='请选择'>
        <PlusOutlined />
      </Popover>
    );
  }
    ...
    if (item.type === 'input') {
      ...
      return (
        <Input
          ...
          onChange={e => this.updateData(e.target.value, r, i, item.dataIndex)}
          disabled={r.disabled}
          addonAfter={this.createPlusBtn(item, r, i)}
        />
      )
      ...
      let controlDisabled = !_.isUndefined(controlledField) && (_.isUndefined(currentControlledValue) || currentControlledValue === item.controlledDisabledValue);

      if (item.dataIndex === 'field_type') {
        const labelMapping = this.state.labelMapping || {};
        controlDisabled = labelMapping[r.label] && labelMapping[r.label].is_edit !== 1;
      }
      return (
        <Select
          size={cellSize || 'default'}
          ...
          disabled={controlDisabled || r.disabled}
        >
          {
            _.isArray(options) ? (
      ...
      return (
        <DatePicker
          ...
          value={t ? moment(t * 1) : ''}
          allowClear
          onChange={e => this.updateData(e ? moment(e).format('X') * 1000 : '', r, i, item.dataIndex)}
          style={{width: '100%'}}
          format={format}
        />
      )
  ...
  updateData = (v, r, i, key, attrs = {}) => {
    ...
    const labelMapping = this.state.labelMapping || {};
    if (key === 'label' && labelMapping[v]) {
      attrs.field_type = labelMapping[v].field_type;
    }
  ...
  render() {
    ...
    const { hiddenTableName, scroll, addFieldName, hiddenSearch, extraBtn, moreDel, style, hiddenHeader, isReadOnly } = this.props;
    const rowSelection = {
      selectedRowKeys,
    ...
    return <>
      <div style={{marginBottom: 10, display: 'flex', justifyContent: 'space-between'}}>
        <div style={{ display: 'flex' }}>
          ...
          {!isReadOnly && <Button disabled={isSearch} style={{margin: '0px 10px'}} onClick={this.addData} type='primary' icon={<PlusOutlined />}>添加{addFieldName || '字段'}</Button>}
          {moreDel && (
