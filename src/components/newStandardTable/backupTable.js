...
// const VirtualTableBody = ({ data, columns, scrollY }) => {
//   const containerRef = useRef(); const startIndex = Math.max(0, Math.floor(scrollY / ROW_HEIGHT) - BUFFER_SIZE);
//   const endIndex = Math.min(data.length, startIndex + Math.ceil(scrollY / ROW_HEIGHT) + BUFFER_SIZE * 2);
//   return (
//     <tbody ref={containerRef}>
//       <tr style={{ height: startIndex * ROW_HEIGHT }} aria-hidden="true" />
//       {data.slice(startIndex, endIndex).map((record, index) => (
//         <tr key={record.key} style={{ height: ROW_HEIGHT }}>
//           {columns.map(col => (
//             <td key={col.dataIndex}>{record[col.dataIndex]}</td>
//           ))}
//         </tr>
//       ))}
//       <tr style={{ height: (data.length - endIndex) * ROW_HEIGHT }} aria-hidden="true" />
//     </tbody>
//   );
// };
...
const ResizeAbleTitle = props => {
  ...
  return (
    <Resizable width={width} handle={<span className='react-resizable-handle' onClick={e => e.stopPropagation()} />} draggableOpts={{enableUserSelectHack: true}} height={0} onResize={onResize}>
      <th {...restProps} />
    </Resizable>
  );
};

const DraggableHeaderCell = ({ index, column, moveColumn, createOverlay }) => {
  const ref = useRef()
...
class StandardTable extends PureComponent {
  ...
  initBaseState = (props) => {
    ...
    this.tableKey = tableKey || columns.map(i => i.dataIndex).join('*') + path;
    const tableConfigStr = '';
  ...
  handleMenuClick = (e, column) => {
    ...
    this.setState(newState, () => {
      ...
      setStorageData(this.tableKey, JSON.stringify({columns: changeColumns.length > 0 ? changeColumns : cls, pageSize}))
    })
  }
  ...
  moveColumn = (item1, item2) => {
    ...
    this.setState({columns: cls}, () => {
      setStorageData(this.tableKey, JSON.stringify({columns: cls, pageSize}))
    })
  }
  ...
  handleTableChange = (p, f, s) => {
    const { columns, sorter, pageSize } = this.state;
    const newColumns = [];
    for (const i of columns) {
    ...
    this.setState({columns: newColumns, current: s.order !== sorter.order || s.field !== sorter.field ? 1 : p.current, pageSize: p.pageSize, sorter: s}, () => {
      this.getTableData();
      if (p.pageSize !== pageSize) {
        setStorageData(this.tableKey, JSON.stringify({columns: newColumns, pageSize: p.pageSize}))
      }
    })
  };
  ...
  callbackData = (res, tableParams) => {
    ...
    if (!this._isUnmounted) {
      this.setState({...state, total: res.total || 0})
    }
  ...
  createColumnRender = (t, r, i, col) => {
    ...
    if (col.ellipsis) {
      return <Tooltip title={v}><Dropdown trigger={['contextMenu']} overlay={this.createCellOverlay(col, r, t)}><div style={{width: col.width - 18}} className='whiteSpace'>{v}</div></Dropdown></Tooltip>
    }
  ...
  getComponent = () => {
    ...
    const { disabledMoveRow } = this.props;
    if (!disabledMoveRow) {
      components.body = {
  ...
  getScroll = () => {
    ...
    const y = height ? getDistance(height, 40) : null;
    return {x: tableWidth === xScroll ? null : uXScroll, y}
  }
  ...
  updateColumns = (state) => {
    ...
    this.setState(state, () => {
      setStorageData(this.tableKey, JSON.stringify({columns: state.columns, pageSize}))
    })
  }

  render () {
    const { selectedRowKeys, columns, data, searchData, isLoading, pageSize, current, total, setDialogVisible, tableWidth } = this.state;
    const { onRow, rowClassName, onChange, onRowClick, hiddenRowKey, activeClass, components, loading, isDraggable, moveRow, pagePosition, setDetailFields, detailFields,
    expandedRowKeys, onSelectRow, childrenColumnName, rowSelectionType, expandedRowRender, onExpandedRowsChange, showHeader, noPager, size, showTotal, height, ...tableProps } = this.props;
    ...
    this.draggableColumns = columns.filter(i => !i.hidden).map((col, index) => ({
      ...
      filterIcon: col.filtered ? <Icon style={{fontSize: 14}} type='filter' theme='fill' className='custom_standard_table_header_filter_active' /> : <Icon style={{fontSize: 14}} type='filter' theme='fill' />,
      title: <DraggableHeaderCell column={col} index={index} createOverlay={this.createOverlay} moveColumn={this.moveColumn} />,
    ...
    const otherProps = {};
    const localExpandable = {};
    if (expandedRowKeys) localExpandable.expandedRowKeys = expandedRowKeys;
    if (expandedRowRender) {
      localExpandable.expandedRowRender = expandedRowRender
      localExpandable.fixed = 'left';
      localExpandable.columnWidth = 40;
    }
      ...
      <DndProvider backend={HTML5Backend}>
        ...
        {setDialogVisible && (
          <SetColumnsDialog
            ...
            handleModalVisible={this.hideSetTableTitleVisible}
          />
        )}
      </DndProvider>
    );
